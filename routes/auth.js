...
router.post('/sregister/:id',async (req,res)=>{
    try{
        ...
        const student=await mastudentModel.findOne({username:username});
        const bid=req.params.id;
        if(!student){
            const newstudent=new mastudentModel({name:name,username:username,password:password,previouspoint:previouspoint});
...
router.post('/mosregister/:id',async (req,res)=>{
    try{
        const{name,username,password,currentyear,mofirstpoint,mosecondpoint,mothirdpoint,mofourthpoint}=req.body;
        const student=await mostudentModel.findOne({username:username});
        const bid=req.params.id;
        if(!student){
            const newstudent=new mostudentModel({name:name,username:username,password:password,currentyear:currentyear,mofirstpoint:mofirstpoint,mosecondpoint:mosecondpoint,mothirdpoint:mothirdpoint,mofourthpoint:mofourthpoint,mototal:(parseInt(mofirstpoint)+parseInt(mosecondpoint)+parseInt(mothirdpoint)+parseInt(mofourthpoint))});
            const s=await newstudent.save();
            const batch=await mobatchModel.findById(bid);
            batch.students.push(s._id);
            await batch.save();
            res.send({msg:'student registration successful'});
        }else{
...
router.post('/slogin',async (req,res)=>{
    try{
        ...
        const student=await mastudentModel.findOne({username:username,password:password});
        if(student){
...
router.post('/moslogin',async (req,res)=>{
    try{
        const{username,password}=req.body;
        const student=await mostudentModel.findOne({username:username,password:password});
        if(student){
            const stoken=jwt.sign({
                id:student._id,
                usename:student.username
