...
const mastudentModel = require('../db/models/mastudentModel');
const mostudentModel = require('../db/models/mostudentModel');
const courseModel=require('../db/models/courseModel');
const coursecertificateModel=require('../db/models/coursecertificateModel');
const mobatchModel=require('../db/models/mobatchModel');
...
router.post('/addCourse',async (req,res)=>{
    try{
        ...
        if(!course){
        const newcourse=new courseModel({name:name,duration:duration,credit:credit,website:website});
...
router.post('/moaddBatch',async (req,res)=>{
    try{
        ...
        const newbatch=new mobatchModel({batch:batch});
        await newbatch.save();
...
router.post('/addMoCertificate/',async (req,res)=>{
    try{
        ...
        const s=await mostudentModel.findById(sid);
        const course=await courseModel.findOne({name:name});
        const newcertificate= new coursecertificateModel({name:name,imageurl:imageurl,s_id:sid,website:course.website,point:course.credit,duration:course.duration});
...
router.put('/verifyMoCertificate',async(req,res)=>{
    try{
        const sid=req.query.sid;
        const cid=req.query.cid;
        const c=await coursecertificateModel.findByIdAndUpdate(cid,{isverified:true,ispending:false});
        const s=await mostudentModel.findById(sid);
        await mostudentModel.findByIdAndUpdate(sid,{mocurrentpoint:s.mocurrentpoint+c.point});
...
router.put('/rejectMoCertificate',async(req,res)=>{
    try{
        ...
        const cid=req.query.cid;
        const c=await coursecertificateModel.findByIdAndUpdate(cid,{isverified:false,ispending:false,remark:remark});
...
router.put('/finalMoUpdate/:id',async(req,res)=>{
    try{
        ...
        const s= await mostudentModel.findById(sid);
        const allr=await coursecertificateModel.findOne({s_id: sid,ispending:true});
        if(allr){
            return res.send({msg:"check all pending requests"});
        }
        if(s.currentyear==1){
            await mostudentModel.findByIdAndUpdate(sid,{currentyear:s.currentyear+1,mototal:s.mototal+s.mocurrentpoint,mofirstpoint:s.mocurrentpoint,mocurrentpoint:0,});
        }else if(s.currentyear==2){
            await mostudentModel.findByIdAndUpdate(sid,{currentyear:s.currentyear+1,mototal:s.mototal+s.mocurrentpoint,mosecondpoint:s.mocurrentpoint,mocurrentpoint:0,});
        }else if(s.currentyear==3){
            await mostudentModel.findByIdAndUpdate(sid,{currentyear:s.currentyear+1,mototal:s.mototal+s.mocurrentpoint,mothirdpoint:s.mocurrentpoint,mocurrentpoint:0,});
        }else if(s.currentyear==4){
            await mostudentModel.findByIdAndUpdate(sid,{mototal:s.mototal+s.mocurrentpoint,mofourthpoint:s.mocurrentpoint,mocurrentpoint:0});
        }
        
        
        await mostudentModel.findByIdAndUpdate(sid,{moocsCertificates:[]});
...
router.delete('/modeleteStudent/',async(req,res)=>{
    try{
        ...
        const sid=req.query.sid;
        const b=await mobatchModel.findById(bid);
        b.students.pull(sid);
        await b.save();
        await mostudentModel.findByIdAndDelete(sid);
...
router.post('/addActivity',async (req,res)=>{
    try{
        ...
        if(!activity){
        const newactivity=new activityModel({name:name,minpoint:minpoint,maxpoint:maxpoint})
...
router.post('/addCertificate/:id',async (req,res)=>{
    try{
        const{name,imageurl}=req.body;
        const sid=req.params.id;
        const student = await mastudentModel.aggregate([
            {
                $match: {
        ...
        const v=await mastudentModel.findById(sid);
        ...
        student[0].activityCountObj.forEach(async(element) => {
            if(element.name==name && element.point==element.maxpoint){
                ...
                flag=false;
                const y=await activitycountModel.findByIdAndUpdate(element._id,{point:element.point+activity.minpoint});
                const newcertificate=new cetificateModel({name:name,imageurl:imageurl,point:activity.minpoint,countid:y._id});
        ...
        if(flag==true){
        const newacount=new activitycountModel({name:name,point:activity.minpoint,maxpoint:activity.maxpoint});
...
router.delete('/deleteCertificate/',async(req, res)=>{
    try{
        ...
        await s.save();
        await activitycountModel.findByIdAndUpdate(c.countid,{point:acount.point-c.point});
...
router.put('/verifyCertificate',async(req,res)=>{
    try{
        const sid=req.query.sid;
        const cid=req.query.cid;
        const c=await cetificateModel.findByIdAndUpdate(cid,{isverified:true,ispending:false});
        const s=await studentModel.findById(sid);
        await studentModel.findByIdAndUpdate(sid,{currentpoint:s.currentpoint+c.point});
        res.send({msg:"certificate verified"});
    }catch(err){
        res.send({err:"server error"});
    }
...
router.put('/rejectCertificate',async(req,res)=>{
    try{
        const {remark}=req.body;
        const cid=req.query.cid;
        const c=await cetificateModel.findByIdAndUpdate(cid,{isverified:false,ispending:false,remark:remark});
        res.send({msg:"certificate rejected"});
    }catch(err){
        res.send({err:"server error"});
    }
...
router.put('/finalUpdate/:id',async(req,res)=>{
    try{
        const sid=req.params.id;
        const s= await studentModel.findById(sid);
        await studentModel.findByIdAndUpdate(sid,{previouspoint:s.previouspoint+s.currentpoint,currentpoint:0});
